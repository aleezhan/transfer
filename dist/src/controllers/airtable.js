import { IntersectionFieldType, } from "./enums.js";
import axios from "axios";
import { Optional } from "typescript-optional";
import airtable from "airtable";
import { utilPrint } from "../utils/print.js";
import _ from "lodash";
export var AirtableAction;
(function (AirtableAction) {
    AirtableAction[AirtableAction["CREATE"] = 0] = "CREATE";
    AirtableAction[AirtableAction["UPDATE"] = 1] = "UPDATE";
})(AirtableAction || (AirtableAction = {}));
export const getAllAirtableRecords = async (apiKey, baseId, tableId, lookBackPeriod, returnFieldsByFieldId) => {
...
export const createAirtableUpdateFieldsGivenFieldMapping = async (record, fieldMapping) => {
...
export const createComparableAirtableRecord = async (sourceAirtable, fieldMapping, primaryKey, primaryKeyValue) => {
    ...
    Object.entries(fieldMapping).forEach(([, fieldMappingValue]) => {
        ...
        switch (fieldType) {
            case IntersectionFieldType.people: {
                const outputArray = [];
                currentAirtable[key]?.forEach((value) => {
...
export async function mutateAirtableRecord(event, airtableAction, airtableRecordId, baseId, tableName, apiKey) {
...
export async function pushToAirtableForGivenFieldMapping(comparableCollection, fieldMapping, baseId, tableId, apiKey, airtableRecordId) {
    ...
    let maybeAirtableRecordId = Optional.ofNullable(airtableRecordId);
