import { createComparableAirtableRecord, getAllAirtableRecords, pushToAirtableForGivenFieldMapping, } from "./airtable.js";
import { utilPrint } from "../utils/print.js";
import { MILLISECONDS_PER_HOUR } from "../utils/time.js";
import { pushToGoogle } from "./google.js";
...
export async function getDataMapGivenDataSource(lookBackPeriodInMilliseconds, dataSource) {
    let result = new Map();
...
export async function getComparableRecord(source, sourceRecord, FIELD_MAPPING) {
    let result = new Map();
    switch (source) {
        case DataSourceType.airtable: {
...
export async function pushToDataSource(comparableCollection, recordId, fieldMapping, dataSource) {
    if (dataSource.configuration.writable) {
        switch (dataSource.type) {
            ...
            case DataSourceType.google: {
                const googleData = dataSource.connection_details;
                if (!(googleData?.base_id) || !(googleData?.table_id) || !(googleData?.api_key)) {
                    throw new Error(`missing client_email (${googleData?.base_id}) or sheet_id (${googleData?.table_id}) or private_key for google sheets`);
                }
                await pushToGoogle(comparableCollection, recordId, googleData.base_id, googleData.api_key, googleData.table_id);
                break;
            }
            default: {
                throw new Error('No data source given');
